...
RUN echo "xdebug.idekey=PHP_STORM" >> $XDEBUGINI_PATH && \
    echo "xdebug.discover_client_host=0" >> $XDEBUGINI_PATH && \
    echo "xdebug.client_host=host.docker.internal" >> $XDEBUGINI_PATH && \
    echo "xdebug.mode=debug,profile,trace" >> $XDEBUGINI_PATH && \
    echo "xdebug.start_with_request=trigger" >> $XDEBUGINI_PATH && \
    echo "xdebug.default_enable=0" >> $XDEBUGINI_PATH && \
    echo "xdebug.remote_enable=0" >> $XDEBUGINI_PATH && \
