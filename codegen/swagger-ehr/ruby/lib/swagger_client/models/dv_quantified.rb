...
module SwaggerClient
  class DvQuantified
    ...
    attr_accessor :other_reference_ranges

    attr_accessor :magnitude_status
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DvQuantified` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DvQuantified`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'_type')
        ...
        self._type = 'DV_QUANTIFIED'
      end

      if attributes.key?(:'normal_status')
        self.normal_status = attributes[:'normal_status']
      end

      if attributes.key?(:'normal_range')
        self.normal_range = attributes[:'normal_range']
      end

      if attributes.key?(:'other_reference_ranges')
        if (value = attributes[:'other_reference_ranges']).is_a?(Array)
          self.other_reference_ranges = value
        end
      end

      if attributes.key?(:'magnitude_status')
        self.magnitude_status = attributes[:'magnitude_status']
      else
    ...
    def ==(o)
      ...
      self.class == o.class &&
          _type == o._type &&
          normal_status == o.normal_status &&
          normal_range == o.normal_range &&
          other_reference_ranges == o.other_reference_ranges &&
          magnitude_status == o.magnitude_status && super(o)
    end
    ...
    def hash
      [_type, normal_status, normal_range, other_reference_ranges, magnitude_status].hash
    end
