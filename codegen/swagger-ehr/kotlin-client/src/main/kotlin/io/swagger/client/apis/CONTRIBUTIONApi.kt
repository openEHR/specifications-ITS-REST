...
import io.swagger.client.models.Contribution
import io.swagger.client.models.Error
import io.swagger.client.models.NewContribution

import io.swagger.client.infrastructure.*

class CONTRIBUTIONApi(basePath: kotlin.String = "https://{baseUrl}/v1") : ApiClient(basePath) {
    ...
    fun contributionCreate(body: NewContribution, ehrId: kotlin.String, prefer: kotlin.String? = null): Contribution {
        ...
        val localVariableConfig = RequestConfig(
                RequestMethod.POST,
                "/ehr/{ehr_id}/contribution".replace("{" + "ehr_id" + "}", "$ehrId"), headers = localVariableHeaders
        )
        val response = request<Contribution>(
                localVariableConfig, localVariableBody
        )

        return when (response.responseType) {
            ResponseType.Success -> (response as Success<*>).data as Contribution
    ...
    fun contributionGet(ehrId: kotlin.String, contributionUid: kotlin.String): Contribution {
        ...
        val localVariableConfig = RequestConfig(
                RequestMethod.GET,
                "/ehr/{ehr_id}/contribution/{contribution_uid}".replace("{" + "ehr_id" + "}", "$ehrId").replace("{" + "contribution_uid" + "}", "$contributionUid")
        )
        val response = request<Contribution>(
                localVariableConfig
        )
