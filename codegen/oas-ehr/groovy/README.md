...
```groovy
def apiInstance = new CompositionApi()
def ehrId = "7d44b88c-4199-4bad-97dc-d78268e01398" // String | EHR identifier taken from EHR.ehr_id.value. 
def composition = new Composition() // Composition | The COMPOSITION. 
def prefer = "return=representation" // String | Request header to indicate the preference over response details. The response will contain the entire resource when the `Prefer` header has a value of `return=representation`. 

apiInstance.compositionCreate(ehrId, composition, prefer)
    {
    // on success
    def result = (Composition)it
    println result
