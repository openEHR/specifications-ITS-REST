...
export class EHRApi {
    ...
    public setApiKey(key: EHRApiApiKeys, value: string) {
        (this.authentications as any)[EHRApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async ehrCreate (prefer?: 'return=representation' | 'return=minimal', ehrStatus?: EhrStatus, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Ehr;  }> {
        const localVarPath = this.basePath + '/ehr';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Ehr;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Ehr");
     ...
     * @param ehrStatus An EHR_STATUS resource MAY be also supplied by the client as the request body. 
     */
    public async ehrCreateWithId (ehrId: string, prefer?: 'return=representation' | 'return=minimal', ehrStatus?: EhrStatus, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Ehr;  }> {
        const localVarPath = this.basePath + '/ehr/{ehr_id}'
     ...
     * @param ehrId EHR identifier taken from EHR.ehr_id.value. 
     */
    public async ehrGetById (ehrId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Ehr;  }> {
        const localVarPath = this.basePath + '/ehr/{ehr_id}'
            .replace('{' + 'ehr_id' + '}', encodeURIComponent(String(ehrId)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
     ...
     */
    public async ehrGetBySubject (subjectId: string, subjectNamespace: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Ehr;  }> {
        const localVarPath = this.basePath + '/ehr';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        if (subjectNamespace !== undefined) {
            localVarQueryParameters['subject_namespace'] = ObjectSerializer.serialize(subjectNamespace, "string");
        }
