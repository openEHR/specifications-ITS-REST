...
export enum CONTRIBUTIONApiApiKeys {
}
...
export class CONTRIBUTIONApi {
    ...
    public setApiKey(key: CONTRIBUTIONApiApiKeys, value: string) {
        (this.authentications as any)[CONTRIBUTIONApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async contributionCreate (ehrId: string, newContribution: NewContribution, prefer?: 'return=representation' | 'return=minimal', options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Contribution;  }> {
        const localVarPath = this.basePath + '/ehr/{ehr_id}/contribution'
        ...
        // verify required parameter 'newContribution' is not null or undefined
        if (newContribution === null || newContribution === undefined) {
            throw new Error('Required parameter newContribution was null or undefined when calling contributionCreate.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(newContribution, "NewContribution")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Contribution;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Contribution");
     ...
     */
    public async contributionGet (ehrId: string, contributionUid: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Contribution;  }> {
        const localVarPath = this.basePath + '/ehr/{ehr_id}/contribution/{contribution_uid}'
            .replace('{' + 'ehr_id' + '}', encodeURIComponent(String(ehrId)))
