...
class ContentItem {
  ...
  @override
  String toString() => 'ContentItem[type=$type]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ContentItem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ContentItem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<ContentItem>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ContentItem.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
