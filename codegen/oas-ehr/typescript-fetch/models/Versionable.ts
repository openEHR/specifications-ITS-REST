...
/**
 * Check if a given object implements the Versionable interface.
 */
export function instanceOfVersionable(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function VersionableFromJSON(json: any): Versionable {
    return VersionableFromJSONTyped(json, false);
}

export function VersionableFromJSONTyped(json: any, ignoreDiscriminator: boolean): Versionable {
    return json;
}

export function VersionableToJSON(value?: Versionable | null): any {
    return value;
}
