...
import type { DvEncapsulated } from './DvEncapsulated';
import {
    DvEncapsulatedFromJSON,
    DvEncapsulatedFromJSONTyped,
    DvEncapsulatedToJSON,
} from './DvEncapsulated';
import type { DvIdentifier } from './DvIdentifier';
import {
    DvIdentifierFromJSON,
    DvIdentifierFromJSONTyped,
    DvIdentifierToJSON,
} from './DvIdentifier';
import type { FeederAuditDetails } from './FeederAuditDetails';
import {
    FeederAuditDetailsFromJSON,
    FeederAuditDetailsFromJSONTyped,
    FeederAuditDetailsToJSON,
} from './FeederAuditDetails';
...
/**
 * Check if a given object implements the FeederAudit interface.
 */
export function instanceOfFeederAudit(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "originatingSystemAudit" in value;

    return isInstance;
}

export function FeederAuditFromJSON(json: any): FeederAudit {
    return FeederAuditFromJSONTyped(json, false);
}

export function FeederAuditFromJSONTyped(json: any, ignoreDiscriminator: boolean): FeederAudit {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'originatingSystemItemIds': !exists(json, 'originating_system_item_ids') ? undefined : ((json['originating_system_item_ids'] as Array<any>).map(DvIdentifierFromJSON)),
        'feederSystemItemIds': !exists(json, 'feeder_system_item_ids') ? undefined : ((json['feeder_system_item_ids'] as Array<any>).map(DvIdentifierFromJSON)),
        'originalContent': !exists(json, 'original_content') ? undefined : DvEncapsulatedFromJSON(json['original_content']),
        'originatingSystemAudit': FeederAuditDetailsFromJSON(json['originating_system_audit']),
        'feederSystemAudit': !exists(json, 'feeder_system_audit') ? undefined : FeederAuditDetailsFromJSON(json['feeder_system_audit']),
    };
}

export function FeederAuditToJSON(value?: FeederAudit | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'originating_system_item_ids': value.originatingSystemItemIds === undefined ? undefined : ((value.originatingSystemItemIds as Array<any>).map(DvIdentifierToJSON)),
        'feeder_system_item_ids': value.feederSystemItemIds === undefined ? undefined : ((value.feederSystemItemIds as Array<any>).map(DvIdentifierToJSON)),
        'original_content': DvEncapsulatedToJSON(value.originalContent),
        'originating_system_audit': FeederAuditDetailsToJSON(value.originatingSystemAudit),
        'feeder_system_audit': FeederAuditDetailsToJSON(value.feederSystemAudit),
    };
}
