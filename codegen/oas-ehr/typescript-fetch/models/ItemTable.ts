...
/**
 * Check if a given object implements the ItemTable interface.
 */
export function instanceOfItemTable(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function ItemTableFromJSON(json: any): ItemTable {
    return ItemTableFromJSONTyped(json, false);
}

export function ItemTableFromJSONTyped(json: any, ignoreDiscriminator: boolean): ItemTable {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        ...ItemStructureFromJSONTyped(json, ignoreDiscriminator),
        'type': !exists(json, '_type') ? undefined : json['_type'],
        'items': !exists(json, 'items') ? undefined : ((json['items'] as Array<any>).map(ClstrFromJSON)),
    };
}

export function ItemTableToJSON(value?: ItemTable | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...ItemStructureToJSON(value),
        '_type': value.type,
        'items': value.items === undefined ? undefined : ((value.items as Array<any>).map(ClstrToJSON)),
    };
}
