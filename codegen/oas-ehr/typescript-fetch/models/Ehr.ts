...
import type { HierObjectId } from './HierObjectId';
import {
    HierObjectIdFromJSON,
...
/**
 * Check if a given object implements the Ehr interface.
 */
export function instanceOfEhr(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function EhrFromJSON(json: any): Ehr {
    return EhrFromJSONTyped(json, false);
}

export function EhrFromJSONTyped(json: any, ignoreDiscriminator: boolean): Ehr {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'systemId': !exists(json, 'system_id') ? undefined : HierObjectIdFromJSON(json['system_id']),
        'ehrId': !exists(json, 'ehr_id') ? undefined : HierObjectIdFromJSON(json['ehr_id']),
        'ehrStatus': !exists(json, 'ehr_status') ? undefined : ObjectRefFromJSON(json['ehr_status']),
        'ehrAccess': !exists(json, 'ehr_access') ? undefined : ObjectRefFromJSON(json['ehr_access']),
        'timeCreated': !exists(json, 'time_created') ? undefined : DvDateTimeFromJSON(json['time_created']),
    };
}

export function EhrToJSON(value?: Ehr | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'system_id': HierObjectIdToJSON(value.systemId),
        'ehr_id': HierObjectIdToJSON(value.ehrId),
        'ehr_status': ObjectRefToJSON(value.ehrStatus),
        'ehr_access': ObjectRefToJSON(value.ehrAccess),
        'time_created': DvDateTimeToJSON(value.timeCreated),
    };
}
