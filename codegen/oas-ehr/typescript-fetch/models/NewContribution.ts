...
import {
    UpdateVersionFromJSON,
    UpdateVersionFromJSONTyped,
    UpdateVersionToJSON,
} from './UpdateVersion';
...
/**
 * Check if a given object implements the NewContribution interface.
 */
export function instanceOfNewContribution(value: object): boolean {
    let isInstance = true;
...
export function NewContributionFromJSON(json: any): NewContribution {
    return NewContributionFromJSONTyped(json, false);
}

export function NewContributionFromJSONTyped(json: any, ignoreDiscriminator: boolean): NewContribution {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'uid': !exists(json, 'uid') ? undefined : HierObjectIdFromJSON(json['uid']),
        'versions': ((json['versions'] as Array<any>).map(UpdateVersionFromJSON)),
        'audit': UpdateAuditFromJSON(json['audit']),
    };
}

export function NewContributionToJSON(value?: NewContribution | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'uid': HierObjectIdToJSON(value.uid),
        'versions': ((value.versions as Array<any>).map(UpdateVersionToJSON)),
        'audit': UpdateAuditToJSON(value.audit),
    };
}
