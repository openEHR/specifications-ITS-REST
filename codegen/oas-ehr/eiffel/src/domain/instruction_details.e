...
feature --Access
    ...
    instruction_id: detachable LOCATABLE_REF
      
    activity_id: detachable STRING_32
      
    wf_details: detachable ITEM_STRUCTURE
      

feature -- Change Element

    set_instruction_id (a_name: like instruction_id)
        -- Set 'instruction_id' with 'a_name'.
      do
        instruction_id := a_name
      ensure
        instruction_id_set: instruction_id = a_name
      end

    set_activity_id (a_name: like activity_id)
        -- Set 'activity_id' with 'a_name'.
      do
        activity_id := a_name
      ensure
        activity_id_set: activity_id = a_name
      end

    set_wf_details (a_name: like wf_details)
        -- Set 'wf_details' with 'a_name'.
      do
        wf_details := a_name
      ensure
        wf_details_set: wf_details = a_name
      end
    ...
    output: STRING
      ...
      do
        ...
        if attached instruction_id as l_instruction_id then
          ...
          Result.append (l_instruction_id.out)
          Result.append ("%N")
        end
        if attached activity_id as l_activity_id then
          Result.append ("%Nactivity_id:")
          Result.append (l_activity_id.out)
          Result.append ("%N")
        end
        if attached wf_details as l_wf_details then
          Result.append ("%Nwf_details:")
          Result.append (l_wf_details.out)
          Result.append ("%N")
        end
      end
end
