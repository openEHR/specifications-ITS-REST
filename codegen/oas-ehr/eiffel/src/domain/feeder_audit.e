...
feature --Access
    ...
    originating_system_item_ids: detachable LIST [DV_IDENTIFIER]
      
    feeder_system_item_ids: detachable LIST [DV_IDENTIFIER]
      
    original_content: detachable DV_ENCAPSULATED
      
    originating_system_audit: detachable FEEDER_AUDIT_DETAILS
      
    feeder_system_audit: detachable FEEDER_AUDIT_DETAILS
      

feature -- Change Element

    set_originating_system_item_ids (a_name: like originating_system_item_ids)
        -- Set 'originating_system_item_ids' with 'a_name'.
      do
        originating_system_item_ids := a_name
      ensure
        originating_system_item_ids_set: originating_system_item_ids = a_name
      end

    set_feeder_system_item_ids (a_name: like feeder_system_item_ids)
        -- Set 'feeder_system_item_ids' with 'a_name'.
      do
        feeder_system_item_ids := a_name
      ensure
        feeder_system_item_ids_set: feeder_system_item_ids = a_name
      end

    set_original_content (a_name: like original_content)
        -- Set 'original_content' with 'a_name'.
      do
        original_content := a_name
      ensure
        original_content_set: original_content = a_name
      end

    set_originating_system_audit (a_name: like originating_system_audit)
        -- Set 'originating_system_audit' with 'a_name'.
      do
        originating_system_audit := a_name
      ensure
        originating_system_audit_set: originating_system_audit = a_name
      end

    set_feeder_system_audit (a_name: like feeder_system_audit)
        -- Set 'feeder_system_audit' with 'a_name'.
      do
        feeder_system_audit := a_name
      ensure
        feeder_system_audit_set: feeder_system_audit = a_name
      end
 ...
 feature -- Status Report
    ...
    output: STRING
      ...
      do
        ...
        Result.append("%Nclass FEEDER_AUDIT%N")
        if attached originating_system_item_ids as l_originating_system_item_ids then
          across l_originating_system_item_ids as ic loop
            Result.append ("%N originating_system_item_ids:")
            Result.append (ic.item.out)
            Result.append ("%N")
          end
        end
        if attached feeder_system_item_ids as l_feeder_system_item_ids then
          across l_feeder_system_item_ids as ic loop
            Result.append ("%N feeder_system_item_ids:")
            Result.append (ic.item.out)
            Result.append ("%N")
          end
        end
        if attached original_content as l_original_content then
          Result.append ("%Noriginal_content:")
          Result.append (l_original_content.out)
          Result.append ("%N")
        end
        if attached originating_system_audit as l_originating_system_audit then
          Result.append ("%Noriginating_system_audit:")
          Result.append (l_originating_system_audit.out)
          Result.append ("%N")
        end
        if attached feeder_system_audit as l_feeder_system_audit then
          Result.append ("%Nfeeder_system_audit:")
          Result.append (l_feeder_system_audit.out)
          Result.append ("%N")
        end
      end
end
