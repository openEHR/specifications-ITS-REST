...
feature -- API Access
	...
	composition_create (ehr_id: STRING_32; composition: COMPOSITION; prefer: STRING_32): detachable COMPOSITION
			-- Create COMPOSITION
			-- Creates the first version of a new COMPOSITION in the EHR identified by &#x60;ehr_id&#x60;. 
			-- 
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: composition The COMPOSITION.  (required)
			-- 
			-- argument: prefer Request header to indicate the preference over response details. The response will contain the entire resource when the &#x60;Prefer&#x60; header has a value of &#x60;return&#x3D;representation&#x60;.  (optional, default to return=minimal)
			-- 
			-- 
			-- Result COMPOSITION
		require
		...
		do
			reset_error
			create l_request
			l_request.set_body(composition)
			l_path := "/ehr/{ehr_id}/composition"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))

			if attached prefer as l_prefer then
				l_request.add_header(l_prefer.out,"Prefer");
			end
			...
			elseif attached { COMPOSITION } l_response.data ({ COMPOSITION }) as l_data then
				Result := l_data
			else
	...
	composition_delete (ehr_id: STRING_32; uid_based_id: STRING_32)
			...
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: uid_based_id An identifier in a form of an OBJECT_VERSION_ID identifier taken from VERSION.uid.value (i.e. a &#x60;version_uid&#x60;).  (required)
			-- 
			-- 
		require
		local
		...
		do
			...
			l_path := "/ehr/{ehr_id}/composition/{uid_based_id}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"uid_based_id"+"}", api_client.url_encode (uid_based_id.out))
	...
	composition_get (ehr_id: STRING_32; uid_based_id: STRING_32; version_at_time: STRING_32): detachable COMPOSITION
			-- Get COMPOSITION
			...
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: uid_based_id An abstract identifier: it can take a form of an OBJECT_VERSION_ID identifier taken from VERSION.uid.value (i.e. a &#x60;version_uid&#x60;), or a form of a HIER_OBJECT_ID identifier taken from VERSIONED_OBJECT.uid.value (i.e. a &#x60;versioned_object_uid&#x60;).  (required)
			-- 
			-- argument: version_at_time A given time in the extended ISO 8601 format.  (optional, default to null)
			-- 
			-- 
			-- Result COMPOSITION
		require
		...
		do
			...
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"uid_based_id"+"}", api_client.url_encode (uid_based_id.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "version_at_time", version_at_time));
	...
	composition_update (ehr_id: STRING_32; uid_based_id: STRING_32; if__match: STRING_32; composition: COMPOSITION; prefer: STRING_32): detachable COMPOSITION
			-- Update COMPOSITION
			...
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: uid_based_id An identifier in a form of a HIER_OBJECT_ID identifier taken from VERSIONED_OBJECT.uid.value (i.e. a &#x60;versioned_object_uid&#x60;).  (required)
			-- 
			-- argument: if__match Header to make the request conditional.  Together with &#x60;ETag&#x60; request tag, it helps to prevent simultaneous updates of a resource from overwriting each other (\&quot;mid-air collisions\&quot;). The format is always an &#x60;version_uid&#x60; identifier enclosed by double quotes. The operation will be performed only if the existing latest &#x60;version_uid&#x60; of the resource (i.e. the &#x60;preceding_version_uid&#x60;) matches this header&#39;s value.  (required)
			-- 
			-- argument: composition The new COMPOSITION.  (required)
			-- 
			-- argument: prefer Request header to indicate the preference over response details. The response will contain the entire resource when the &#x60;Prefer&#x60; header has a value of &#x60;return&#x3D;representation&#x60;.  (optional, default to return=minimal)
			-- 
			-- 
		...
		do
			...
			if attached if__match as l_if__match then
				l_request.add_header(l_if__match.out,"If-Match");
			end
			if attached prefer as l_prefer then
				l_request.add_header(l_prefer.out,"Prefer");
			end
	...
	versioned_composition_get (ehr_id: STRING_32; versioned_object_uid: STRING_32): detachable VERSIONED_COMPOSITION
			...
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: versioned_object_uid VERSIONED_COMPOSITION identifier taken from VERSIONED_COMPOSITION.uid.value.  (required)
			-- 
			-- 
			-- Result VERSIONED_COMPOSITION
		require
		local
  			l_path: STRING
  			l_request: API_CLIENT_REQUEST
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_composition/{versioned_object_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"versioned_object_uid"+"}", api_client.url_encode (versioned_object_uid.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { VERSIONED_COMPOSITION } l_response.data ({ VERSIONED_COMPOSITION }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	versioned_composition_revision_history (ehr_id: STRING_32; versioned_object_uid: STRING_32): detachable REVISION_HISTORY
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_composition/{versioned_object_uid}/revision_history"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"versioned_object_uid"+"}", api_client.url_encode (versioned_object_uid.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { REVISION_HISTORY } l_response.data ({ REVISION_HISTORY }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	versioned_composition_version_get_at_time (ehr_id: STRING_32; versioned_object_uid: STRING_32; version_at_time: STRING_32): detachable VERSION
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_composition/{versioned_object_uid}/version"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"versioned_object_uid"+"}", api_client.url_encode (versioned_object_uid.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "version_at_time", version_at_time));
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { VERSION } l_response.data ({ VERSION }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	versioned_composition_version_get_by_id (ehr_id: STRING_32; versioned_object_uid: STRING_32; version_uid: STRING_32): detachable VERSION
			...
			-- argument: versioned_object_uid VERSIONED_COMPOSITION identifier taken from VERSIONED_COMPOSITION.uid.value.  (required)
			-- 
			-- argument: version_uid VERSION identifier taken from VERSION.uid.value.  (required)
			-- 
			-- 
			-- Result VERSION
		require
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_composition/{versioned_object_uid}/version/{version_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"versioned_object_uid"+"}", api_client.url_encode (versioned_object_uid.out))
			l_path.replace_substring_all ("{"+"version_uid"+"}", api_client.url_encode (version_uid.out))
