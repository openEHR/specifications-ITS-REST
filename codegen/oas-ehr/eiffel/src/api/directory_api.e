...
feature -- API Access
	...
	directory_create (ehr_id: STRING_32; folder: FOLDER; prefer: STRING_32): detachable FOLDER
			-- Create directory
			-- Creates a new directory FOLDER associated with the EHR identified by &#x60;ehr_id&#x60;. 
			-- 
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: folder The directory.  (required)
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(folder)
			l_path := "/ehr/{ehr_id}/directory"
			...
			elseif attached { FOLDER } l_response.data ({ FOLDER }) as l_data then
				Result := l_data
			else
	...
	directory_get_at_time (ehr_id: STRING_32; version_at_time: STRING_32; path: STRING_32): detachable FOLDER
			...
			-- 
			-- argument: path A path to a sub-folder; consists of slash-separated values of the name attribute of FOLDERs in the directory.  (optional, default to null)
			-- 
			-- 
			-- Result FOLDER
		require
	...
	directory_get_by_version_id (ehr_id: STRING_32; version_uid: STRING_32; path: STRING_32): detachable FOLDER
		...
		do
			...
			l_path := "/ehr/{ehr_id}/directory/{version_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
	...
	directory_update (ehr_id: STRING_32; if__match: STRING_32; folder: FOLDER; prefer: STRING_32): detachable FOLDER
			-- Update directory
			...
			-- argument: if__match Header to make the request conditional.  Together with &#x60;ETag&#x60; request tag, it helps to prevent simultaneous updates of a resource from overwriting each other (\&quot;mid-air collisions\&quot;). The format is always an &#x60;version_uid&#x60; identifier enclosed by double quotes. The operation will be performed only if the existing latest &#x60;version_uid&#x60; of the resource (i.e. the &#x60;preceding_version_uid&#x60;) matches this header&#39;s value.  (required)
			-- 
			-- argument: folder The new directory.  (required)
			-- 
			-- argument: prefer Request header to indicate the preference over response details. The response will contain the entire resource when the &#x60;Prefer&#x60; header has a value of &#x60;return&#x3D;representation&#x60;.  (optional, default to return=minimal)
			-- 
			-- 
