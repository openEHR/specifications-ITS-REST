...
feature -- API Access
	...
	contribution_create (ehr_id: STRING_32; new_contribution: NEW_CONTRIBUTION; prefer: STRING_32): detachable CONTRIBUTION
			-- Create CONTRIBUTION
			...
			-- 
			-- argument: new_contribution The CONTRIBUTION.  (required)
			-- 
		...
		do
			reset_error
			create l_request
			l_request.set_body(new_contribution)
			l_path := "/ehr/{ehr_id}/contribution"
			...
			elseif attached { CONTRIBUTION } l_response.data ({ CONTRIBUTION }) as l_data then
				Result := l_data
			else
	...
	contribution_get (ehr_id: STRING_32; contribution_uid: STRING_32): detachable CONTRIBUTION
			...
			-- argument: ehr_id EHR identifier taken from EHR.ehr_id.value.  (required)
			-- 
			-- argument: contribution_uid The CONTRIBUTION uid.  (required)
			-- 
			-- 
			-- Result CONTRIBUTION
		require
		...
		do
			...
			l_path := "/ehr/{ehr_id}/contribution/{contribution_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"contribution_uid"+"}", api_client.url_encode (contribution_uid.out))
