...
feature -- API Access
	...
	ehr_create (prefer: STRING_32; ehr_status: detachable EHR_STATUS): detachable EHR
			-- Create EHR
			...
			-- 
			-- argument: ehr_status An EHR_STATUS resource MAY be also supplied by the client as the request body.  (optional)
			-- 
			-- 
			-- Result EHR
		require
		...
		do
			...
			elseif attached { EHR } l_response.data ({ EHR }) as l_data then
				Result := l_data
			else
	...
	ehr_create_with_id (ehr_id: STRING_32; prefer: STRING_32; ehr_status: detachable EHR_STATUS): detachable EHR
			-- Create EHR with id
	...
	ehr_get_by_subject (subject_id: STRING_32; subject_namespace: STRING_32): detachable EHR
			...
			-- 
			-- argument: subject_id The EHR subject id.  (required)
			-- 
			-- argument: subject_namespace The EHR subject id namespace.  (required)
			-- 
			-- 
			-- Result EHR
		require
		...
		do
			...
			l_path := "/ehr"
			l_request.fill_query_params(api_client.parameter_to_tuple("", "subject_id", subject_id));
			l_request.fill_query_params(api_client.parameter_to_tuple("", "subject_namespace", subject_namespace));
