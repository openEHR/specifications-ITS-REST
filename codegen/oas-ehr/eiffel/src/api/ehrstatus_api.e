...
feature -- API Access
	...
	ehr_status_get_at_time (ehr_id: STRING_32; version_at_time: STRING_32): detachable EHR_STATUS
		...
		do
			...
			elseif attached { EHR_STATUS } l_response.data ({ EHR_STATUS }) as l_data then
				Result := l_data
			else
	...
	ehr_status_get_by_version_id (ehr_id: STRING_32; version_uid: STRING_32): detachable EHR_STATUS
		...
		do
			...
			l_path := "/ehr/{ehr_id}/ehr_status/{version_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
	...
	ehr_status_update (ehr_id: STRING_32; if__match: STRING_32; ehr_status: EHR_STATUS; prefer: STRING_32): detachable EHR_STATUS
			-- Update EHR_STATUS
			...
			-- 
			-- argument: ehr_status The new EHR_STATUS.  (required)
			-- 
	...
	versioned_ehr_status_get (ehr_id: STRING_32): detachable VERSIONED_EHR_STATUS
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_ehr_status"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			...
			if l_response.has_error then
				last_error := l_response.error
			elseif attached { VERSIONED_EHR_STATUS } l_response.data ({ VERSIONED_EHR_STATUS }) as l_data then
				Result := l_data
			else
				create last_error.make ("Unknown error: Status response [ " + l_response.status.out + "]")
			end
	...
	versioned_ehr_status_revision_history (ehr_id: STRING_32): detachable REVISION_HISTORY
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_ehr_status/revision_history"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
	...
	versioned_ehr_status_version_get_at_time (ehr_id: STRING_32; version_at_time: STRING_32): detachable VERSION
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_ehr_status/version"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_request.fill_query_params(api_client.parameter_to_tuple("", "version_at_time", version_at_time));
	...
	versioned_ehr_status_version_get_by_id (ehr_id: STRING_32; version_uid: STRING_32): detachable VERSION
		...
		do
			...
			l_path := "/ehr/{ehr_id}/versioned_ehr_status/version/{version_uid}"
			l_path.replace_substring_all ("{"+"ehr_id"+"}", api_client.url_encode (ehr_id.out))
			l_path.replace_substring_all ("{"+"version_uid"+"}", api_client.url_encode (version_uid.out))
