...
#
package WWW::OpenAPIClient::CONTRIBUTIONApi;
...
# @param NewContribution $new_contribution The CONTRIBUTION.  (required)
# @param string $prefer Request header to indicate the preference over response details. The response will contain the entire resource when the &#x60;Prefer&#x60; header has a value of &#x60;return&#x3D;representation&#x60;.  (optional, default to 'return=minimal')
{
    ...
    __PACKAGE__->method_documentation->{ 'contribution_create' } = {
...
# @return Contribution
#
sub contribution_create {
    my ($self, %args) = @_;

    # verify the required parameter 'ehr_id' is set
    unless (exists $args{'ehr_id'}) {
      croak("Missing the required parameter 'ehr_id' when calling contribution_create");
    }

    # verify the required parameter 'new_contribution' is set
    unless (exists $args{'new_contribution'}) {
      croak("Missing the required parameter 'new_contribution' when calling contribution_create");
    }

    # parse inputs
    my $_resource_path = '/ehr/{ehr_id}/contribution';
    ...
    my $_body_data;
    # body params
    if ( exists $args{'new_contribution'}) {
        $_body_data = $args{'new_contribution'};
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('Contribution', $response);
...
# @param string $contribution_uid The CONTRIBUTION uid.  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'contribution_get' } = {
...
# @return Contribution
#
sub contribution_get {
    my ($self, %args) = @_;

    # verify the required parameter 'ehr_id' is set
    unless (exists $args{'ehr_id'}) {
      croak("Missing the required parameter 'ehr_id' when calling contribution_get");
    }

    # verify the required parameter 'contribution_uid' is set
    unless (exists $args{'contribution_uid'}) {
      croak("Missing the required parameter 'contribution_uid' when calling contribution_get");
    }

    # parse inputs
    my $_resource_path = '/ehr/{ehr_id}/contribution/{contribution_uid}';
    ...
    # path params
    if ( exists $args{'contribution_uid'}) {
        my $_base_variable = "{" . "contribution_uid" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'contribution_uid'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }
