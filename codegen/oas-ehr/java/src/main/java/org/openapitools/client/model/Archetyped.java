...
public class Archetyped {
  ...
  * @throws IOException if the JSON Element is invalid with respect to Archetyped
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Archetyped.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Archetyped.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Archetyped` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      ArchetypeId.validateJsonElement(jsonObj.get("archetype_id"));
      // validate the optional field `template_id`
      if (jsonObj.get("template_id") != null && !jsonObj.get("template_id").isJsonNull()) {
        TemplateId.validateJsonElement(jsonObj.get("template_id"));
      }
