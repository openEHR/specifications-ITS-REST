...
public class DvDateTime extends DataValue {
  ...
  protected String type = "DV_DATE_TIME";
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvDateTime
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvDateTime.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvDateTime is not found in the empty JSON string", DvDateTime.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvDateTime.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvDateTime` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DvDateTime.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvDateTime.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvDateTime' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvDateTime> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvDateTime.class));

       return (TypeAdapter<T>) new TypeAdapter<DvDateTime>() {
           @Override
           public void write(JsonWriter out, DvDateTime value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvDateTime read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvDateTime given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvDateTime
  * @throws IOException if the JSON string is invalid with respect to DvDateTime
  */
  public static DvDateTime fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvDateTime.class);
  }

 /**
  * Convert an instance of DvDateTime to an JSON string
  *
  * @return JSON string
  */
