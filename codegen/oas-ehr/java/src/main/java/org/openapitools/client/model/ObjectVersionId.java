...
public class ObjectVersionId extends UidBasedId {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ObjectVersionId
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ObjectVersionId.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ObjectVersionId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjectVersionId` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
