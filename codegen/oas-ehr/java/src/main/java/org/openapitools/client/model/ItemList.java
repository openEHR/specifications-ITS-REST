...
public class ItemList extends ItemStructure {
  ...
  protected String type = "ITEM_LIST";
  ...
  * @throws IOException if the JSON Object is invalid with respect to ItemList
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ItemList.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ItemList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ItemList` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  }
