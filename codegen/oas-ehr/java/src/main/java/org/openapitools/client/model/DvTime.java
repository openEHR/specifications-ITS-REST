...
public class DvTime extends DvTemporal {
  ...
  * @throws IOException if the JSON Element is invalid with respect to DvTime
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DvTime.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DvTime.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvTime` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
