...
public class DvEhrUri extends DvUri {
  ...
  protected String type = "DV_EHR_URI";
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvEhrUri
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvEhrUri.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvEhrUri is not found in the empty JSON string", DvEhrUri.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvEhrUri.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvEhrUri` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvEhrUri.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvEhrUri' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvEhrUri> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvEhrUri.class));

       return (TypeAdapter<T>) new TypeAdapter<DvEhrUri>() {
           @Override
           public void write(JsonWriter out, DvEhrUri value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvEhrUri read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvEhrUri given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvEhrUri
  * @throws IOException if the JSON string is invalid with respect to DvEhrUri
  */
  public static DvEhrUri fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvEhrUri.class);
  }

 /**
  * Convert an instance of DvEhrUri to an JSON string
  *
  * @return JSON string
  */
