...
public class Clstr extends Item {
  ...
  * @throws IOException if the JSON Object is invalid with respect to Clstr
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!Clstr.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Clstr is not found in the empty JSON string", Clstr.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Clstr.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Clstr` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Clstr.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Clstr.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Clstr' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Clstr> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Clstr.class));

       return (TypeAdapter<T>) new TypeAdapter<Clstr>() {
           @Override
           public void write(JsonWriter out, Clstr value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Clstr read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of Clstr given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of Clstr
  * @throws IOException if the JSON string is invalid with respect to Clstr
  */
  public static Clstr fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Clstr.class);
  }

 /**
  * Convert an instance of Clstr to an JSON string
  *
  * @return JSON string
  */
