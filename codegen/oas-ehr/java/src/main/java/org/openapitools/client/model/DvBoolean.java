...
public class DvBoolean extends DataValue {
  ...
  protected String type = "DV_BOOLEAN";
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvBoolean
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvBoolean.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvBoolean is not found in the empty JSON string", DvBoolean.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvBoolean.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvBoolean` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DvBoolean.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvBoolean.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvBoolean' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvBoolean> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvBoolean.class));

       return (TypeAdapter<T>) new TypeAdapter<DvBoolean>() {
           @Override
           public void write(JsonWriter out, DvBoolean value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvBoolean read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvBoolean given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvBoolean
  * @throws IOException if the JSON string is invalid with respect to DvBoolean
  */
  public static DvBoolean fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvBoolean.class);
  }

 /**
  * Convert an instance of DvBoolean to an JSON string
  *
  * @return JSON string
  */
