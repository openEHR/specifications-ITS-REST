...
public class ArchetypeId extends ObjectId {
  ...
  protected String type = "ARCHETYPE_ID";
  ...
  * @throws IOException if the JSON Object is invalid with respect to ArchetypeId
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ArchetypeId.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ArchetypeId is not found in the empty JSON string", ArchetypeId.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ArchetypeId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ArchetypeId` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ArchetypeId.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ArchetypeId.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ArchetypeId' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ArchetypeId> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ArchetypeId.class));

       return (TypeAdapter<T>) new TypeAdapter<ArchetypeId>() {
           @Override
           public void write(JsonWriter out, ArchetypeId value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ArchetypeId read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ArchetypeId given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ArchetypeId
  * @throws IOException if the JSON string is invalid with respect to ArchetypeId
  */
  public static ArchetypeId fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ArchetypeId.class);
  }

 /**
  * Convert an instance of ArchetypeId to an JSON string
  *
  * @return JSON string
  */
