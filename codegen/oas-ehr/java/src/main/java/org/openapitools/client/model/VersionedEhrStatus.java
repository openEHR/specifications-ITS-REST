...
public class VersionedEhrStatus extends VersionedObject {
  ...
  * @throws IOException if the JSON Element is invalid with respect to VersionedEhrStatus
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VersionedEhrStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VersionedEhrStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VersionedEhrStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
