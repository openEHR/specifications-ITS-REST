...
import org.openapitools.client.model.RevisionHistoryItem;
...
public class RevisionHistory {
  ...
  * @throws IOException if the JSON Object is invalid with respect to RevisionHistory
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!RevisionHistory.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in RevisionHistory is not found in the empty JSON string", RevisionHistory.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!RevisionHistory.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RevisionHistory` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : RevisionHistory.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        RevisionHistoryItem.validateJsonObject(jsonArrayitems.get(i).getAsJsonObject());
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!RevisionHistory.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'RevisionHistory' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<RevisionHistory> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(RevisionHistory.class));

       return (TypeAdapter<T>) new TypeAdapter<RevisionHistory>() {
           @Override
           public void write(JsonWriter out, RevisionHistory value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public RevisionHistory read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of RevisionHistory given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of RevisionHistory
  * @throws IOException if the JSON string is invalid with respect to RevisionHistory
  */
  public static RevisionHistory fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, RevisionHistory.class);
  }

 /**
  * Convert an instance of RevisionHistory to an JSON string
  *
  * @return JSON string
  */
