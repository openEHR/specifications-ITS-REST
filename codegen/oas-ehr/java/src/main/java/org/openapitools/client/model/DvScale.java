...
public class DvScale extends DvOrdered {
  ...
  protected String type = "DV_SCALE";
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvScale
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvScale.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvScale is not found in the empty JSON string", DvScale.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvScale.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvScale` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DvScale.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvScale.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvScale' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvScale> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvScale.class));

       return (TypeAdapter<T>) new TypeAdapter<DvScale>() {
           @Override
           public void write(JsonWriter out, DvScale value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvScale read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvScale given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvScale
  * @throws IOException if the JSON string is invalid with respect to DvScale
  */
  public static DvScale fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvScale.class);
  }

 /**
  * Convert an instance of DvScale to an JSON string
  *
  * @return JSON string
  */
