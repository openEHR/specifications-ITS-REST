...
public class DvScale extends DvOrdered {
  ...
  * @throws IOException if the JSON Element is invalid with respect to DvScale
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DvScale.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DvScale.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvScale` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
