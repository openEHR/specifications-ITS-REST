...
public class DvCodedText extends DvText {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvCodedText
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvCodedText.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvCodedText.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvCodedText` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
