...
public class GenericId extends ObjectId {
  ...
  * @throws IOException if the JSON Element is invalid with respect to GenericId
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GenericId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GenericId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GenericId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
