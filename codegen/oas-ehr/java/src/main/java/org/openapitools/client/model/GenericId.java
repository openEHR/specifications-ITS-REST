...
public class GenericId extends ObjectId {
  ...
  protected String type = "GENERIC_ID";
  ...
  * @throws IOException if the JSON Object is invalid with respect to GenericId
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!GenericId.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GenericId is not found in the empty JSON string", GenericId.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!GenericId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GenericId` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : GenericId.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GenericId.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GenericId' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GenericId> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GenericId.class));

       return (TypeAdapter<T>) new TypeAdapter<GenericId>() {
           @Override
           public void write(JsonWriter out, GenericId value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GenericId read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of GenericId given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of GenericId
  * @throws IOException if the JSON string is invalid with respect to GenericId
  */
  public static GenericId fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GenericId.class);
  }

 /**
  * Convert an instance of GenericId to an JSON string
  *
  * @return JSON string
  */
