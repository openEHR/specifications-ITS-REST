...
public class HierObjectId extends UidBasedId {
  ...
  * @throws IOException if the JSON Element is invalid with respect to HierObjectId
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HierObjectId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HierObjectId is not found in the empty JSON string", HierObjectId.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HierObjectId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HierObjectId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : HierObjectId.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HierObjectId.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HierObjectId' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HierObjectId> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HierObjectId.class));

       return (TypeAdapter<T>) new TypeAdapter<HierObjectId>() {
           @Override
           public void write(JsonWriter out, HierObjectId value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HierObjectId read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of HierObjectId given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of HierObjectId
  * @throws IOException if the JSON string is invalid with respect to HierObjectId
  */
  public static HierObjectId fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HierObjectId.class);
  }

 /**
  * Convert an instance of HierObjectId to an JSON string
  *
  * @return JSON string
  */
