...
public class DvDate extends DvTemporal {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvDate
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvDate.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvDate is not found in the empty JSON string", DvDate.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvDate.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvDate` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DvDate.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvDate.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvDate' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvDate> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvDate.class));

       return (TypeAdapter<T>) new TypeAdapter<DvDate>() {
           @Override
           public void write(JsonWriter out, DvDate value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvDate read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvDate given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvDate
  * @throws IOException if the JSON string is invalid with respect to DvDate
  */
  public static DvDate fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvDate.class);
  }

 /**
  * Convert an instance of DvDate to an JSON string
  *
  * @return JSON string
  */
