...
public class DvIdentifier extends DataValue {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DvIdentifier
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DvIdentifier.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DvIdentifier is not found in the empty JSON string", DvIdentifier.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DvIdentifier.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DvIdentifier` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DvIdentifier.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DvIdentifier.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DvIdentifier' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DvIdentifier> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DvIdentifier.class));

       return (TypeAdapter<T>) new TypeAdapter<DvIdentifier>() {
           @Override
           public void write(JsonWriter out, DvIdentifier value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DvIdentifier read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DvIdentifier given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DvIdentifier
  * @throws IOException if the JSON string is invalid with respect to DvIdentifier
  */
  public static DvIdentifier fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DvIdentifier.class);
  }

 /**
  * Convert an instance of DvIdentifier to an JSON string
  *
  * @return JSON string
  */
