...
import org.openapitools.client.model.AdminEntry;
...
public class AdminEntry extends AbstractEntry {
  ...
  protected String type = "ADMIN_ENTRY";
  ...
  * @throws IOException if the JSON Object is invalid with respect to AdminEntry
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!AdminEntry.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AdminEntry is not found in the empty JSON string", AdminEntry.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AdminEntry.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AdminEntry` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AdminEntry.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AdminEntry.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AdminEntry' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AdminEntry> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AdminEntry.class));

       return (TypeAdapter<T>) new TypeAdapter<AdminEntry>() {
           @Override
           public void write(JsonWriter out, AdminEntry value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AdminEntry read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of AdminEntry given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of AdminEntry
  * @throws IOException if the JSON string is invalid with respect to AdminEntry
  */
  public static AdminEntry fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AdminEntry.class);
  }

 /**
  * Convert an instance of AdminEntry to an JSON string
  *
  * @return JSON string
  */
