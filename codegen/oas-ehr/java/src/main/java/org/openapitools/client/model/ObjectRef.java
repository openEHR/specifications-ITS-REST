...
public class ObjectRef {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ObjectRef
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ObjectRef.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ObjectRef is not found in the empty JSON string", ObjectRef.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ObjectRef.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjectRef` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ObjectRef.openapiRequiredFields) {
      ...
      ObjectId.validateJsonObject(jsonObj.getAsJsonObject("id"));
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ObjectRef.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ObjectRef' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ObjectRef> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ObjectRef.class));

       return (TypeAdapter<T>) new TypeAdapter<ObjectRef>() {
           @Override
           public void write(JsonWriter out, ObjectRef value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ObjectRef read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ObjectRef given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ObjectRef
  * @throws IOException if the JSON string is invalid with respect to ObjectRef
  */
  public static ObjectRef fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ObjectRef.class);
  }

 /**
  * Convert an instance of ObjectRef to an JSON string
  *
  * @return JSON string
  */
