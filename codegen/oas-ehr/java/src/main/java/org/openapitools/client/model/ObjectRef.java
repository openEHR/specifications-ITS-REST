...
public class ObjectRef {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ObjectRef
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ObjectRef.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ObjectRef.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjectRef` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      ObjectId.validateJsonElement(jsonObj.get("id"));
  }
