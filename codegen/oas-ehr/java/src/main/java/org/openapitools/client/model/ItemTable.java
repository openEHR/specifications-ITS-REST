...
public class ItemTable extends ItemStructure {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ItemTable
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ItemTable.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ItemTable.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ItemTable` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  }
