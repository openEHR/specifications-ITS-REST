...
import org.openapitools.client.model.PartyRelated;
...
public class PartyRelated extends PartyIdentified {
  ...
  protected String type = "PARTY_RELATED";
  ...
  * @throws IOException if the JSON Object is invalid with respect to PartyRelated
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!PartyRelated.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PartyRelated is not found in the empty JSON string", PartyRelated.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PartyRelated.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartyRelated` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PartyRelated.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PartyRelated.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PartyRelated' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PartyRelated> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PartyRelated.class));

       return (TypeAdapter<T>) new TypeAdapter<PartyRelated>() {
           @Override
           public void write(JsonWriter out, PartyRelated value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PartyRelated read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of PartyRelated given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of PartyRelated
  * @throws IOException if the JSON string is invalid with respect to PartyRelated
  */
  public static PartyRelated fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PartyRelated.class);
  }

 /**
  * Convert an instance of PartyRelated to an JSON string
  *
  * @return JSON string
  */
