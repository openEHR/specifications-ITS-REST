...
public class PartyRef {
  ...
  * @throws IOException if the JSON Object is invalid with respect to PartyRef
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!PartyRef.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PartyRef is not found in the empty JSON string", PartyRef.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PartyRef.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartyRef` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PartyRef.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PartyRef.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PartyRef' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PartyRef> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PartyRef.class));

       return (TypeAdapter<T>) new TypeAdapter<PartyRef>() {
           @Override
           public void write(JsonWriter out, PartyRef value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PartyRef read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of PartyRef given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of PartyRef
  * @throws IOException if the JSON string is invalid with respect to PartyRef
  */
  public static PartyRef fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PartyRef.class);
  }

 /**
  * Convert an instance of PartyRef to an JSON string
  *
  * @return JSON string
  */
