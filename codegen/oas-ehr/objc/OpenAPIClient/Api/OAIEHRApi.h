...
@interface OAIEHRApi: NSObject <OAIApi>

extern NSString* kOAIEHRApiErrorDomain;
extern NSInteger kOAIEHRApiMissingParamErrorCode;
...
/// 
///  code:201 message:"`201 Created` is returned when the EHR has been successfully created.  The new EHR resource is returned in the body when the request's `Prefer` header value is `return=representation`, otherwise only headers are returned. ",
...
///  code:409 message:"`409 Conflict` Unable to create a new EHR due to a conflict with an already existing EHR with the same subject id, namespace pair, whenever EHR_STATUS is supplied. "
///
/// @return OAIEhr*
...
///  code:201 message:"`201 Created` is returned when the EHR has been successfully created.  The new EHR resource is returned in the body when the request's `Prefer` header value is `return=representation`, otherwise only headers are returned. ",
///  code:400 message:"`400 Bad Request` is returned when the request URL or body (if provided) could not be parsed or has invalid content. ",
///  code:409 message:"`409 Conflict` Unable to create a new EHR due to a conflict with an already existing EHR.  Can happen when the supplied `ehr_id` is already used by an existing EHR. "
///
/// @return OAIEhr*
...
/// @param ehrId EHR identifier taken from EHR.ehr_id.value. 
/// 
///  code:200 message:"`200 OK` is returned when the requested EHR resource is successfully retrieved. ",
...
/// 
///  code:200 message:"`200 OK` is returned when the requested EHR resource is successfully retrieved. ",
///  code:404 message:"`404 Not Found` is returned when an EHR with supplied subject parameters does not exist. "
///
/// @return OAIEhr*
