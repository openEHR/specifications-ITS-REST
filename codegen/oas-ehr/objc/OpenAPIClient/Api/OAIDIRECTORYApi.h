...
@interface OAIDIRECTORYApi: NSObject <OAIApi>

extern NSString* kOAIDIRECTORYApiErrorDomain;
extern NSInteger kOAIDIRECTORYApiMissingParamErrorCode;
...
/// 
///  code:201 message:"`201 Created` is returned when the FOLDER was created.  Content body is only returned when `Prefer` header has `return=representation`, otherwise only headers are returned. ",
///  code:400 message:"`400 Bad Request` is returned when the request URL or body (if provided) could not be parsed or has invalid content (e.g. invalid `ehr_id`, or either the body of the request could not be read, or could not be converted to a valid FOLDER object). ",
...
/// 
///  code:204 message:"`204 No Content` is returned when the resource identified by the request parameters has been (logically) deleted. ",
///  code:400 message:"`400 Bad Request` is returned when the request has invalid `ehr_id`. ",
///  code:404 message:"`404 Not Found` is returned when an EHR with `ehr_id` does not exist. ",
...
/// 
///  code:200 message:"`200 OK` is returned when the FOLDER is successfully retrieved. ",
...
///  code:404 message:"`404 Not Found` is returned when an EHR with `ehr_id` does not exist, or when a directory does not exist at the specified `version_at_time`, or when `path` does not exists within the directory. "
///
/// @return OAIFolder*
...
/// 
///  code:200 message:"`200 OK` is returned when the FOLDER is successfully retrieved. ",
///  code:404 message:"`404 Not Found` is returned when an EHR with `ehr_id` does not exist, or when a directory with `version_uid` does not exist, or when `path` does not exist within the directory. "
///
/// @return OAIFolder*
...
/// @param prefer Request header to indicate the preference over response details. The response will contain the entire resource when the &#x60;Prefer&#x60; header has a value of &#x60;return&#x3D;representation&#x60;.  (optional) (default to @"return=minimal")
/// 
///  code:200 message:"`200 OK` is returned when directory was successfully updated and the updated resource is returned in the body when `Prefer` header value is `return=representation`. ",
///  code:204 message:"`204 No Content` is returned when directory was updated and `Prefer` header is missing or is set to `return=minimal`. ",
///  code:400 message:"`400 Bad Request` is returned when the request URL or body (if provided) could not be parsed or has invalid content (e.g. invalid `ehr_id`, or either the body of the request could not be read, or could not be converted to a valid FOLDER object). ",
///  code:404 message:"`404 Not Found` is returned when an EHR with `ehr_id` does not exist. ",
///  code:412 message:"`412 Precondition Failed` is returned when `If-Match` request header doesn't match the latest version on the service side. Returns also latest `version_uid` in the `Location` and `ETag` headers. "
///
