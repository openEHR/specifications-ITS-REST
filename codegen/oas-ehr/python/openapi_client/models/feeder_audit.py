...
class FeederAudit(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["originating_system_item_ids", "feeder_system_item_ids", "original_content", "originating_system_audit", "feeder_system_audit"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in originating_system_item_ids (list)
        _items = []
        if self.originating_system_item_ids:
            for _item in self.originating_system_item_ids:
                if _item:
                    _items.append(_item.to_dict())
            _dict['originating_system_item_ids'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in feeder_system_item_ids (list)
        _items = []
        if self.feeder_system_item_ids:
            for _item in self.feeder_system_item_ids:
                if _item:
                    _items.append(_item.to_dict())
            _dict['feeder_system_item_ids'] = _items
        # override the default output from pydantic by calling `to_dict()` of original_content
        if self.original_content:
            _dict['original_content'] = self.original_content.to_dict()
        # override the default output from pydantic by calling `to_dict()` of originating_system_audit
        if self.originating_system_audit:
            _dict['originating_system_audit'] = self.originating_system_audit.to_dict()
        # override the default output from pydantic by calling `to_dict()` of feeder_system_audit
        if self.feeder_system_audit:
            _dict['feeder_system_audit'] = self.feeder_system_audit.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "originating_system_item_ids": [DvIdentifier.from_dict(_item) for _item in obj.get("originating_system_item_ids")] if obj.get("originating_system_item_ids") is not None else None,
            "feeder_system_item_ids": [DvIdentifier.from_dict(_item) for _item in obj.get("feeder_system_item_ids")] if obj.get("feeder_system_item_ids") is not None else None,
            "original_content": DvEncapsulated.from_dict(obj.get("original_content")) if obj.get("original_content") is not None else None,
            "originating_system_audit": FeederAuditDetails.from_dict(obj.get("originating_system_audit")) if obj.get("originating_system_audit") is not None else None,
            "feeder_system_audit": FeederAuditDetails.from_dict(obj.get("feeder_system_audit")) if obj.get("feeder_system_audit") is not None else None
        })
        return _obj
