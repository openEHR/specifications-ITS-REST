...
class Attestation(AuditDetails):
    ...
    Attestation
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default='ATTESTATION', alias="_type")
    ...
    __properties: ClassVar[List[str]] = ["_type", "system_id", "time_committed", "change_type", "description", "committer"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of time_committed
        if self.time_committed:
            _dict['time_committed'] = self.time_committed.to_dict()
        # override the default output from pydantic by calling `to_dict()` of change_type
        if self.change_type:
            _dict['change_type'] = self.change_type.to_dict()
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "_type": obj.get("_type") if obj.get("_type") is not None else 'ATTESTATION',
            "system_id": obj.get("system_id"),
            "time_committed": DvDateTime.from_dict(obj.get("time_committed")) if obj.get("time_committed") is not None else None,
            "change_type": DvCodedText.from_dict(obj.get("change_type")) if obj.get("change_type") is not None else None,
            "description": DvText.from_dict(obj.get("description")) if obj.get("description") is not None else None,
            "committer": PartyProxy.from_dict(obj.get("committer")) if obj.get("committer") is not None else None
        })
        return _obj
