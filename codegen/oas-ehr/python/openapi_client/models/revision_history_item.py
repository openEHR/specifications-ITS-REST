...
class RevisionHistoryItem(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["version_id", "audits"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of version_id
        if self.version_id:
            _dict['version_id'] = self.version_id.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in audits (list)
        _items = []
        if self.audits:
            for _item in self.audits:
                if _item:
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "version_id": ObjectVersionId.from_dict(obj.get("version_id")) if obj.get("version_id") is not None else None,
            "audits": [AuditDetails.from_dict(_item) for _item in obj.get("audits")] if obj.get("audits") is not None else None
        })
        return _obj
