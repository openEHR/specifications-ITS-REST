...
class TermMapping(BaseModel):
    ...
    TermMapping
    """ # noqa: E501
    match: Annotated[str, Field(strict=True)]
    purpose: Optional[DvCodedText] = None
    target: CodePhrase
    __properties: ClassVar[List[str]] = ["match", "purpose", "target"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "match": obj.get("match"),
            "purpose": DvCodedText.from_dict(obj.get("purpose")) if obj.get("purpose") is not None else None,
            "target": CodePhrase.from_dict(obj.get("target")) if obj.get("target") is not None else None
        })
        return _obj
