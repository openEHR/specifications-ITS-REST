...
class PartyRef(BaseModel):
    ...
    id: ObjectId
    __properties: ClassVar[List[str]] = ["namespace", "type", "id"]

    @field_validator('type')
    def type_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^PERSON|ORGANISATION|GROUP|AGENT|ROLE|PARTY|ACTOR$", value):
            raise ValueError(r"must validate the regular expression /^PERSON|ORGANISATION|GROUP|AGENT|ROLE|PARTY|ACTOR$/")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type"),
            "id": ObjectId.from_dict(obj.get("id")) if obj.get("id") is not None else None
        })
        return _obj
