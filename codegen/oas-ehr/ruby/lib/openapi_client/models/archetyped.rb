...
module OpenapiClient
  class Archetyped
    attr_accessor :archetype_id

    attr_accessor :template_id

    attr_accessor :rm_version
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Archetyped` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Archetyped`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'archetype_id')
        self.archetype_id = attributes[:'archetype_id']
      else
        self.archetype_id = nil
      end
      ...
      if attributes.key?(:'rm_version')
        self.rm_version = attributes[:'rm_version']
      else
        self.rm_version = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @archetype_id.nil?
        invalid_properties.push('invalid value for "archetype_id", archetype_id cannot be nil.')
      end

      if @rm_version.nil?
        invalid_properties.push('invalid value for "rm_version", rm_version cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @archetype_id.nil?
      return false if @rm_version.nil?
      true
    end
    ...
    def hash
      [archetype_id, template_id, rm_version].hash
    end
