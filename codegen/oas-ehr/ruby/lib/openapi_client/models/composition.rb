  ...
  class Composition < Versionable
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Composition` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Composition`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @territory.nil?
        invalid_properties.push('invalid value for "territory", territory cannot be nil.')
      end
      ...
      if @composer.nil?
        invalid_properties.push('invalid value for "composer", composer cannot be nil.')
      end
    ...
    def hash
      [_type, language, territory, category, context, composer, content].hash
    end
