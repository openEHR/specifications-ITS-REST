...
In the _Standalone launch_, a client _Application_ can request (invoke) a _Platform_ for the context selection. Most Patient and Practitioner facing application, along with the initial login and consent page, will also require the _Platform_ to send additional *context*. This may contain information like the EHR or Patient ID, potentially the Episode ID, and it may be valid for a lifetime of the session.
