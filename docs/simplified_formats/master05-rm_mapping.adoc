...
| no
| Free-text value used when the constraint allows an open value-set (`listOpen: true` in the web template). When `\|other` is set, `\|code`, `\|value` and `\|terminology` MUST NOT be set; the leaf is serialised in the canonical RM as a `DV_TEXT` rather than a `DV_CODED_TEXT`.
...
* `\|other` is mutually exclusive with `\|code`, `\|value` and `\|terminology` on the same leaf;
